...
| **HowItWorks (all screens)** | Orientation drawn as the sequence a task actually is: numbered steps with connectors, a row on a wide screen and a column on a narrow one. Markup rather than a picture, so it reflows, reads aloud in order, and cannot go stale the way an SVG of a renamed button does. Dismissible per screen and remembered, because guidance that cannot be shut up is guidance people learn to scroll past |
