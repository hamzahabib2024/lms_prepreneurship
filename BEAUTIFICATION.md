...
| **Fields say whether they are right** | A green tick when a field is correctly filled, a red cross and a sentence when it is not — done once in `components/Field.tsx` and in the `.field` rules, so it reaches every form rather than fifty files. **Nothing is red on arrival**: the old rule used `:not(:placeholder-shown)` to mean "the user has typed", which is only true of an input that HAS a placeholder — so twelve required inputs across eight screens, including both boxes on the login page, were drawn with an error border before the visitor had touched anything. `:user-invalid` is the selector that means what was intended. The tick is deliberately quieter than the cross: nineteen green badges on the application form would bury the one field that is wrong. The shape carries the meaning and the colour is decoration, because red-on-green is the one pair a twelfth of men cannot separate (NFR-ACC-007) |
...
| **LiveRoomPage** | The class itself, inside the LMS rather than on somebody else's site — the branch ClassPage was always ready for and could never take while Meet forbade framing. A **pre-join screen first**: the student sees themselves, picks their camera and microphone, and decides before a single person can see or hear them, because the alternative is discovering the wrong mic ten minutes in, or arriving on camera by accident in front of the whole class. The self-view is mirrored, like every mirror. A denied camera **does not block the door** — it says so and leaves Join live, since somebody with a broken webcam still has a lesson to attend. Inside: LiveKit's grid, screen share and chat, a reconnecting toast so a patchy line reads as waiting rather than frozen, and full screen for the times a 16:9 frame beside a sidebar is not enough to follow a lesson |
...
| **CohortImportPage** | As above. Now carries the partner picker — asked before the file, never as a CSV column — and states the billing consequence in words before the button is pressed, because under PARTNER_PAYS no charge is raised against any of the imported students and that is invisible until the ledger is empty |
| **EmailQueuePage** | Slate, as the rest of Administration. Two queues on one screen and deliberately unequal: *waiting for you* carries the count in its heading and the only primary button, because it is the only thing here that is a job; *waiting for the mail server* is quiet prose, because it needs nothing from anybody. A single &ldquo;pending&rdquo; number covering both would let the handful that need a decision vanish into the many that do not |
| **PartnersPage** | As above. Institutes as rows rather than a table: the billing mode reads as a sentence on a pill (&ldquo;We invoice the institute&rdquo;) rather than as the enum, and *Give someone an account* is a disclosure on the row it belongs to — it hands an outsider the ability to read student records, so it names the institute in its confirmation and shows the temporary password exactly once |
| **PartnerPortalPage** | The one page a partner institute has. Same shell, same sidebar, same theme as everybody else — read-only throughout, with not one control that changes anything. The Invoices tab does not exist for a STUDENT_PAYS partner rather than showing an empty page, because an empty Invoices page implies there could be invoices |
